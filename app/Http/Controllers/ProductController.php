...
class ProductController extends Controller
{
    ...
    public function manage_product(Request $request,$id='')
    {
        if($id>0){
            ...
            $result['productImagesArr']=DB::table('product_img')->where(['products_id'=>$id])->get();

            $productImagesArr=DB::table('product_img')->where(['products_id'=>$id])->get();
...
//        die();
        ...
        foreach ($skuArr as $key => $val) {
            ...
            if (isset($check[0])) {
                $request->session()->flash('sku_error', $skuArr[$key] . 'SKU already use');
        ...
        foreach ($piidArr as $key => $val) {
            ...
            if ($piidArr[$key] != '') {
                DB::table('product_img')->where(['id' => $piidArr[$key]])->update($productImageArr);
            } else {
                DB::table('product_img')->insert($productImageArr);
            }
        }
    ...
    {
        DB::table('product_img')->where(['id'=>$piid])->delete();
